...
Además, permitiremos declarar una variable a la misma vez que la instanciamos con un valor si no es el \'{a}mbito global. Las funciones siempre deben ser instanciadas al declararse cosa que no era necesar\'{i}a al principio pero para facilitar las etapas posteriores se realiz\'{o} as\'{i}. \footnote{Los ejemplos de los siguientes capítulos son meramente ilustrativos, pueden tener errores
...
Cada par $\{$ $\}$ colocado correctamente marca un ámbito en nuestro lenguaje y recíprocamente, cada ámbito es marcado mediante este par, excepto por el ámbito global. Cada par $\{$ $\}$ debe ser colocado tras un contexto que lo permita, como tras un if + condición o tras la declaración de una función con sus argumentos. Cada ámbito puede tener variables locales, las cuales se pierden al abandonar dicho ámbito, y una tabla de simbolos propia. Además, como es razonable, se permite el anidamiento de ámbitos, es decir, subámbitos de un ámbito más grande, el superámbito, definidos mediante el anidamiento de $\{$ $\}$ o si el superámbito es el total, por incluir  $\{$ $\}$ mientras que el sub\'{a}mbito no sea proveniente de una funci\'{o}n o una definici\'{o}n de clase o struct. En este caso, no hay visibilidad para fuera de estos \'{a}mbitos excepto la declaraci\'{o}n de un m\'{e}todo en una clase y los atributos de la misma. Por tanto, los símbolos del subámbito son un superconjunto de los símbolos del superámbito.
...
Nuestro lenguaje permit\'{i}a el uso de módulos externos, que son meramente archivos de código. Pero al final no se realiz\'{o} ni siquiera al crear el léxico ya que era lo que menos prioridad dimos.
\newpage
\section{Funciones}
Las funciones se declaran con un tipo funcional, un identifcador, una lista de argumentos y el cuerpo de la función (un ámbito acabado en ;). Tambi\'{e}n las funciones pueden ser definidas en cualquier \'{a}mbito aunque no ser\'{a}n visibles fuera del mismo.

Cada programa tiene una función especial de nombre main con tipos de argumentos sin definir hasta que la creemos y tipo retorno void, ya que es solo un símbolo especial. 
