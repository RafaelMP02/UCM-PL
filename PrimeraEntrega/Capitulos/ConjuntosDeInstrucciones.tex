...
\section{Asignación}
Nuestro primer tipo de instrucciones son las de asignación. Como hemos visto a lo largo del documento, hay una sintaxis variada para ello dependiendo del tipo de la variable. Se muestran ejemplos de todo el documento. Recordemos que en el lenguaje no se pueden tener asignaciones en el \'{a}mbito global.

\begin{algorithm}

	\Integer{} X  := \const{2};
	
	\Integer{\operator{*}} p  := \const{null}\;
	...
	\Integer{}(\Integer{}, \Integer{}) potencia(a,b) \{ \\
	\quad \Ifff{(b < \const{0})} \{\\
	\quad \quad \reserved{return} \const{0}\;
	\quad \} \Elsif{(b \operator{==} \const{0})} \{\\
	\quad \quad \reserved{return} \const{1}\;
	\quad \} \Elsee \{\\
	\quad \quad \reserved{return} potencia(a,b\operator{-}\const{1})\;
	\quad \} \\
	\}\;
...
Solo nos queda por comentar la función new que es una función sobrecargada que permite asignarle memoria dinámica a un puntero.
\section{;}
...
\section{For}
El siguiente tipo de instrucciones son las formadas por un for. Se usa la palabra reservada \reserved{for} y la sintaxis es la habitual de C++.
Como antes obligamos a colocar \{ y \} para describir su ámbito. Adem\'{a}s, la condici\'{o}n de paso puede escribirse con los operadores m\'{a}s m\'{a}s y menos menos.
...
\section{Switch}
Vemos ahora el siguiente tipo de instrucción, las formadas por un switch. Usamos las palabras reservadas \reserved{switch},
...
Finalmente el último tipo de instrucción. Para la salida estándar está reservada la palabra \reserved{prints} que imprime el contenido de una variable primitiva por la salida estándar. Para la entrada estándar, tenemos la palabra reservada \reserved{scans} que de igual manera recoge el valor de una variable primitiva dada por la entrada estándar.

\bigskip
Finalmente, mostramos un ejemplo en el que se usan todas las instrucciones para ejemplificar su uso: \footnote{Recordamos que es ilustrativo, en el apartado de generación de código
mostraremos fragmentos que se puedan compilar}
